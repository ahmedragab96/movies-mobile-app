...
export class Theme<T extends Partial<CustomeThemeStyles> = {}> {
  ...
  typography: ThemeTypography<T['typography'] extends undefined
    ? {}
    : Exclude<T['typography'], undefined>> = new ThemeTypography();
  ...
  constructor(t?: RecursivePartialNonMethod<Theme<T>>) {
    if (t) {
      ...
      if (t.typography) {
        this.typography = new ThemeTypography<T['typography'] extends undefined
          ? {}
          : Exclude<T['typography'], undefined>>(t.typography as any);
      }
    }
  }
}
