...
export const ImageField: React.FC<ImageFieldProps> = (props) => {
  ...
  const chooseImage = async () => {
    ...
    const writePermission = PermissionsAndroid.check('android.permission.CAMERA');
    const cameraPermisson = PermissionsAndroid.check('android.permission.WRITE_EXTERNAL_STORAGE');
    ...
    >
      ...
      {imageValue.uri ? (
        ...
        >
          <Image
            source={{
              uri: !imageValue.uri ? imageValue.uri : `data:image/gif;base64,${imageValue.imageData?.fileBase64}`,
            }}
        ...
        : (
          <View style={[selectStyle('container'), selectStyle('error', [haseErrors])]}>
            {!haseErrors && (
            <View
